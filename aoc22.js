...
let p2 = () => {
  ...
  for (let [t, a, b, c, d, e, f] of steps) {
    ...
    let y = +c, dy = +d - c + 1
    let z = +e, dz = +f - e + 1
    let cub = { x, y, z, dx, dy, dz }
    let sub2 = [], add2 = t == 'on' ? [cub] : []
    for (let e of add) if (t = intersect(e, cub)) sub2.push(t)
    for (let e of sub) if (t = intersect(e, cub)) add2.push(t)
    for (let e of add2) add.push(e)
    for (let e of sub2) sub.push(e)
  }
